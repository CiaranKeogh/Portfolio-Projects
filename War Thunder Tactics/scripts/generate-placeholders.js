...
function generatePlaceholderImage(filePath, width, height, label, bgColor = '#333333', textColor = '#ffffff') {
  ...
  // Draw the label (truncate if too long)
...
mapThumbnails.forEach(map => {
  ...
  generatePlaceholderImage(thumbnailPath, 800, 500, `Thumbnail: ${map.label}`, '#2b4c7d', '#ffffff');
  
  // Also create full map image
  const fullMapPath = path.join(baseDir, 'maps', map.name.replace('.jpg', '_full.jpg'));
  generatePlaceholderImage(fullMapPath, 1600, 1000, `Full Map: ${map.label}`, '#2b4c7d', '#ffffff');
  
  // Simple map image for references
  const mapPath = path.join(baseDir, 'maps', map.name);
  generatePlaceholderImage(mapPath, 1200, 800, `Map: ${map.label}`, '#2b4c7d', '#ffffff');
});
...
positionImages.forEach(pos => {
  ...
  generatePlaceholderImage(posPath, 600, 400, pos.label, '#4a6741', '#ffffff');
});
...
avatars.forEach(avatar => {
  ...
  generatePlaceholderImage(avatarPath, 200, 200, avatar.label, '#603e1f', '#ffffff');
});
...
tanks.forEach(tank => {
  ...
  generatePlaceholderImage(tankPath, 400, 300, tank.label, '#3a3a3a', '#ffffff');
});
...
aircraft.forEach(ac => {
  ...
  generatePlaceholderImage(acPath, 400, 300, ac.label, '#3a3a3a', '#ffffff');
});
...
routes.forEach(route => {
  ...
  generatePlaceholderImage(routePath, 800, 600, route.label, '#3d5c3a', '#ffffff');
});
...
generatePlaceholderImage(heroPath, 1920, 1080, 'Hero Background', '#1a2e42', '#ffffff');
